...
export function Study() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-du-bg to-black">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        <header className="rounded-2xl border border-du/30 bg-gradient-to-r from-du/20 to-black/40 p-6 shadow-lg shadow-black/30">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm uppercase tracking-widest text-du/80 font-semibold">Study</p>
              <h1 className="text-3xl font-extrabold text-white">Find your subject</h1>
              <p className="text-gray-400 text-sm mt-1">
                Pick a course, filter by category, and open the materials you need.
              </p>
            </div>
            <div className="inline-flex items-center gap-2 rounded-full bg-du/10 px-4 py-2 text-sm text-du">
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-white">Choose your course</h2>
            {loading && (
              ...
              return (
                <button
                  ...
                  className={`whitespace-nowrap rounded-full border px-4 py-2 text-sm font-medium transition ${
                    active
                      ? 'bg-du text-white border-du shadow-md'
                      : 'border-du/40 text-white/80 hover:bg-du/10'
                  }`}
                >
                  {course}
                </button>
              ...
              return (
                <button
                  ...
                  className={`rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-wide transition ${
                    active
                      ? 'bg-du text-white border-du'
                      : 'border-du/40 text-gray-400 hover:bg-du/10'
                  }`}
                >
                  {tab.label}
                </button>
        ...
        <section className="grid lg:grid-cols-5 gap-6">
          <div className="lg:col-span-2 space-y-4">
            <div
              className="rounded-2xl border border-du/30 bg-black/40 backdrop-blur shadow-xl shadow-black/25 p-4"
              style={{ backgroundColor: 'var(--du-surface)' }}
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <BookOpen className="w-5 h-5 text-du" />
                  <h3 className="text-base font-semibold text-white">Subjects</h3>
                </div>
              ...
              {loading ? (
                <div className="space-y-2">
                  {[...Array(6)].map((_, idx) => (
                    <div key={idx} className="h-12 rounded-xl bg-du/10 animate-pulse"></div>
                  ))}
                </div>
              ) : filteredSubjects.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-10 text-center text-gray-400">
                  <BookOpen className="w-10 h-10 text-du/50 mb-3" />
                    ...
                    return (
                      <button
                        ...
                        className={`w-full rounded-xl border px-4 py-3 text-left transition shadow-sm flex items-center justify-between ${
                          active
                            ? 'bg-du text-white border-du shadow-md'
                            : 'border-du/30 bg-black/30 text-white hover:border-du/60'
                        }`}
                      >
          ...
          <div className="lg:col-span-3 space-y-4">
            <div
              className="rounded-2xl border border-du/30 bg-black/40 backdrop-blur shadow-xl shadow-black/25 overflow-hidden"
              style={{ backgroundColor: 'var(--du-surface)' }}
            >
              <div className="p-6 border-b border-du/20 bg-gradient-to-r from-du/20 to-transparent">
                <p className="text-xs uppercase tracking-wide text-gray-400">Selected subject</p>
              ...
              <div className="p-6">
                {materialsLoading ? (
                  <div className="space-y-4">
                    {[1, 2, 3].map((i) => (
                      <div key={i} className="h-28 bg-du/10 rounded-xl animate-pulse"></div>
                    ))}
                  </div>
                ) : materials.length === 0 ? (
                  <div className="text-center py-12">
                    <BookOpen className="w-12 h-12 text-du/40 mx-auto mb-4" />
                  ...
                  <div className="space-y-4">
                    {materials.map((material) => (
                      <div
                        key={material.id}
                        className="border border-du/25 rounded-xl p-4 bg-black/30 hover:border-du/60 transition"
                      >
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <button className="px-3 py-1 text-xs font-semibold rounded-full bg-du/15 text-du hover:bg-du/25 transition">
                            View
                          </button>
                        </div>
                      </div>
